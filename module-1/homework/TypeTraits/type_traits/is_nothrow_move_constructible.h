...
struct IsNoThrowConstructible
    : LibCppIsNoThrowConstructible<IsConstructible<T, Args...>::value, std::is_reference_v<T>, T, Args...> {
};

template <typename T, std::size_t N>
struct IsNoThrowConstructible<T[N]>
    : LibCppIsNoThrowConstructible<IsConstructible<T>::value, std::is_reference_v<T>, T> {
};

template <typename T>
struct IsNoThrowMoveConstructible
    : public IsNoThrowConstructible<T, typename AddRvalueReference<T>::type> {
};