...
public:
  ...
  friend struct AssignUnion;
  friend struct AccessUnion;
};

struct AssignUnion {
  template <std::size_t Index, typename U, std::size_t UnionIndex, typename Head, typename... Tail>
...
struct AccessUnion {
  template <typename U>
  static constexpr auto&& Get(U&& value, InPlaceIndex<0>) {
...
constexpr std::size_t FindCondition(std::size_t i, std::size_t res, const bool (&founded)[SizeOfFounded],
...
constexpr std::size_t FindPos(std::size_t current_position, const bool (&founded)[SizeOfFounded],
  ...
  return FindCondition(current_position, FindPos(current_position + 1, founded, convertible), founded, convertible);
}
